...
const GitHubPR = ({ branchName, displayName, progress }: { branchName: string; displayName: string; progress: number }) => {
    ...
    <div className="w-full max-w-2xl rounded-lg overflow-hidden border shadow-elevated" style={{ background: '#0d1117', borderColor: '#30363d' }}>
      ...
      <div className="px-4 py-3 border-b" style={{ borderColor: '#30363d' }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[14px] font-semibold" style={{ color: '#58a6ff' }}>portfolio</span>
        </div>
        {prCreated && (
          <div>
      ...
      <div className="px-4 py-3 space-y-2">
        {prCreated && (
          <div className="rounded-md border p-3 space-y-2" style={{ borderColor: '#30363d', background: '#161b22' }}>
            <div className="flex items-center gap-2 text-[13px]">
              <span>{reviewPassed ? <BlueTick /> : <svg width="18" height="18" viewBox="0 0 18 18" className="shrink-0 animate-spin" style={{ color: '#d29922' }}><circle cx="9" cy="9" r="7.5" stroke="currentColor" strokeWidth="2" fill="none" strokeDasharray="35" strokeDashoffset="10" strokeLinecap="round" /></svg>}</span>
              <span style={{ color: '#c9d1d9' }}>CI / Build & Test</span>
              <span className="ml-auto text-[11px]" style={{ color: reviewPassed ? '#3fb950' : '#d29922' }}>
                {reviewPassed ? 'Passed' : 'Running...'}
              </span>
            </div>
            <div className="flex items-center gap-2 text-[13px]">
              <span>{reviewPassed ? <BlueTick /> : <svg width="18" height="18" viewBox="0 0 18 18" className="shrink-0 animate-spin" style={{ color: '#8b949e' }}><circle cx="9" cy="9" r="7.5" stroke="currentColor" strokeWidth="2" fill="none" strokeDasharray="35" strokeDashoffset="10" strokeLinecap="round" /></svg>}</span>
              <span style={{ color: '#c9d1d9' }}>Lint & Type Check</span>
              <span className="ml-auto text-[11px]" style={{ color: reviewPassed ? '#3fb950' : '#8b949e' }}>
                {reviewPassed ? 'Passed' : 'Queued'}
              </span>
            </div>
          </div>
        )}
        ...
        {approved && (
          <div className="rounded-md border p-3 flex items-center gap-2" style={{ borderColor: '#238636', background: '#0d1117' }}>
            <BlueTick />
            <div>
              <div className="text-[13px] font-semibold" style={{ color: '#3fb950' }}>Approved</div>
              <div className="text-[11px]" style={{ color: '#8b949e' }}>austin-jose reviewed and approved</div>
            </div>
          </div>
        )}
        ...
        {approved && (
          <button className="w-full py-2 rounded-md text-[14px] font-semibold transition-all"
            style={{
              background: merged ? '#238636' : '#238636',
              color: '#fff',
